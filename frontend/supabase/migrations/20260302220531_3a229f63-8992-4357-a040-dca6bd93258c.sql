...
-- Create role enum
CREATE TYPE public.app_role AS ENUM ('stajyer', 'personel', 'takim_lideri', 'yonetici', 'ust_yonetici', 'ik');
...
CREATE POLICY "IK can view all profiles"
  ON public.profiles FOR SELECT
  USING (public.has_role(auth.uid(), 'ik'));

CREATE POLICY "Ust yonetici can view all profiles"
  ON public.profiles FOR SELECT
...
CREATE POLICY "IK can manage all roles"
  ON public.user_roles FOR ALL
  USING (public.has_role(auth.uid(), 'ik'));

CREATE POLICY "Ust yonetici can view all roles"
  ON public.user_roles FOR SELECT
  USING (public.has_role(auth.uid(), 'ust_yonetici'));
